...
class Client {
    ...
    joinChat(channelUserorReconnect = false, useridOrReconnect, autoReconnect = false) {
        this.chatService.close();
        if (typeof channelUserorReconnect === 'number') {
            if (typeof useridOrReconnect === 'number') {
                this.chatService.join(channelUserorReconnect, useridOrReconnect, this.client.tokens.access, autoReconnect);
            }
            else {
                this.chatService.join(channelUserorReconnect, this.user.user.userid, this.client.tokens.access, autoReconnect);
            }
        }
        else {
